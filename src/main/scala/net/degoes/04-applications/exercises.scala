...
object exercises extends App {
  ...
  def crawlIO[E: Monoid, A: Monoid](
    ...
    def loop(seeds: Set[URL], ref: Ref[(Crawl[E, A], Set[URL])]): IO[Nothing, Unit] =
      seeds.flatMap(router).foldLeft(IO.unit) { (acc, url) =>
        acc *> getURL(url).redeem(
          _ => IO.unit,
          html =>
...
//    loop(seeds, mzero[Crawl[E, A]], seeds)
  ...
  def crawlIO2[E: Monoid, A: Monoid](
    ...
    getURL    : URL => IO[Exception, String] = getURL): IO[Nothing, Crawl[E, A]] = {
  ...
  def myGame[F[_]: Console: Random: Monad]: F[Unit] =
    for {
      name  <- getName[F]
      word  <- chooseWord[F]
      state <- State(name, Set.empty, word).point[F]
  ...
  for {
    ...
    state <- if (state.playerLost) {
      ...
      printLine[F]("Congrats you have won! now buy yourself a cookie and pat your cat on the head, cause he is smarter than you.") *> state.point[F]
    } else {
      val guessedRight = state.word.contains(guess)
      printLine[F](if (guessedRight) s"Good job, ${state.name}, keep going" else "You are fucking wrong, try again loser.") *> gameLoop[F](state)
    }
  } yield state
  ...
  def getChoice[F[_]: Console: Monad]: F[Char] =
    for {
      ...
      line <- readLine[F]
      letter <- line.trim.toLowerCase.headOption match {
        case Some(letter) =>
          letter.point[F]
        case None =>
  ...
  def chooseWord[F[_]: Random: Functor]: F[String] =
    nextInt[F](Dictionary.length).map(Dictionary.lift(_).getOrElse("buggy"))
  ...
  //
  lazy val myGameIO: IO[Nothing, Unit] = myGame[IO[Nothing, ?]]
